...
// Fallback function to break down tasks without using OpenAI
function generateFallbackSteps(task: string, stepCount: string): { title: string, completed: boolean }[] {
  console.log(`Using fallback breakdown for task: ${task}`);
  const basicSteps = [
    { title: `Gather all materials needed for: ${task}`, completed: false },
    { title: `Prepare your workspace for: ${task}`, completed: false },
    { title: `Start with the first part of: ${task}`, completed: false },
    { title: `Continue with the middle section of: ${task}`, completed: false },
    { title: `Complete the final part of: ${task}`, completed: false },
    { title: `Review your work on: ${task}`, completed: false },
    { title: `Put away materials from: ${task}`, completed: false }
  ];

  // Adjust number of steps based on requested step count
  const count = parseInt(stepCount.split('-')[1]) || 5;
  return basicSteps.slice(0, Math.min(count, basicSteps.length));
}
...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      // Generate fallback steps without using OpenAI
      const fallbackSteps = generateFallbackSteps(task, stepCount);
